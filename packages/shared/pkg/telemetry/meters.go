...
const (
	...
	// Node memory-pressure metrics — from /proc/pressure/memory (PSI).
...
const (
	...
	// UFFDIO_COPY ENOMEM stall — carries outcome=retry_succeeded|retry_exhausted.
	SandboxUffdCopyEnomemTotal CounterType = "orchestrator.sandbox.uffd.copy_enomem.total"
	// Hugepage pool OOM — sandbox died because UFFDIO_COPY could not be served before stall budget elapsed.
...
const (
	...
	// UFFDIO_COPY ENOMEM stall duration — only recorded on the stall path.
...
const (
	...
	// Sandbox host-side physical-memory metrics — read from /proc and cgroup, not envd.
	...
	// Node hugepage pool metrics — from /proc/meminfo.
